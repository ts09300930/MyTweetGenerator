...
if st.button("生成開始"):
    if not features or not API_KEY:
        ...
        # 多様性・奥行き指示（複数日数での繰り返し防止を最強化）
        variety_instruction = """
        すべてのツイートで内容、表現、シチュエーション、言い回し、感情描写を完全に多様化せよ。
        複数日分生成時も、すべてのツイートで表現・フレーズ・誘導・シチュエーションを完全にユニークにせよ。過去ツイートとの重複を絶対禁止。各ツイートは独立した新鮮な内容に。
        ...
        誘導文を毎回完全に異なる自然な表現にせよ。同じフレーズの繰り返しを厳禁（例: 「おじさんしかいないんです…」「今すぐ助けてください…」「こんな私を、おじさんが溶かしてくれないと…」「我慢できないから声かけて…」「垂れた胸を、おじさんの手で…」「この疼き、おじさんだけに…」「今だけは許して…」「こんな私を抱きしめて…」「溶かしてほしいんです…」「リプで温めて…」「優しく溶かしてくれませんか…」「今夜だけは…」など）。
