...
if st.button("生成開始"):
    if not features or not API_KEY:
        ...
        if erotic_level <= 2:
            erotic_instruction = "エロティックさは極めて控えめ。性的表現は一切避け、孤独感や雰囲気の描写のみ。フレーズの繰り返しを絶対禁止。各ツイートで異なる表現を使い、感覚を多角的に。"
        elif erotic_level <= 4:
            erotic_instruction = "エロティックさは軽い暗示レベル。体が熱くなる、ドキドキするなどの間接表現のみ。おじさんの視線や存在を想像させる表現を1つ入れ、止められない欲求をじわっと・びくんなどのオノマトペで具体的に。自己卑下を強く連携（例: 「こんな私でも」）。"
        elif erotic_level <= 6:
            erotic_instruction = "中程度のエロティック表現。疼き、濡れそうな感覚、触れたい衝動などの感覚描写を適度に取り入れる。おじさんの手・息遣い・視線を想像させる表現を必ず1つ以上入れ、部位を多様に（胸・太もも・腰など偏らせず）。強い自己卑下と誘導を組み合わせ、止められない欲求を具体的に描写。フレーズの繰り返しを絶対禁止。"
        elif erotic_level <= 8:
            erotic_instruction = "やや大胆なエロティック表現。指の動き、息遣い、具体的な部位の熱さなどの描写を積極的に。おじさんを意識した視点で、おじさんの存在を想像させる表現を必須に。感覚をぞわぞわ・とろーりなどの生々しいオノマトペで描写し、日常きっかけから欲求が爆発する流れを明確に。自己卑下を強く連携し、むらむらを最大化。"
        else:
            erotic_instruction = "生々しく大胆なエロティック表現。具体的な感覚描写や行為の想像を強く含むが、センシティブ回避ルールを厳守。おじさんの視線・手・存在を想像させる表現を必ず入れ、部位を多様に（胸に偏らず太もも・首筋・指先など）。強い自己卑下（「こんな私でも」「我慢できないんです」）＋切実な誘導を組み合わせ、止められない欲求をじわっと・びくん・ぞわぞわなどの具体的なオノマトペで描写。おじさんの支配・独占欲を刺激する背徳要素（例: 「人目があるのに」「夜中で一人なのに」）を1つ追加。"
