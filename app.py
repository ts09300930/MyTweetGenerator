...
st.title("裏垢女子ツイート生成ツール ver1.00")
...
if st.button("生成開始"):
    if not features or not API_KEY:
        ...
        # エロ度指示（最新強化版）
        if erotic_level <= 2:
            erotic_instruction = "エロティックさは極めて控えめ。性的表現は一切避け、孤独感や雰囲気の描写のみ。フレーズの繰り返しを絶対禁止。完全に自然で流暢な日本語文にせよ。"
        elif erotic_level <= 4:
            erotic_instruction = "エロティックさは軽い暗示レベル。体が熱くなる、ドキドキするなどの間接表現のみ。おじさんの視線や存在を想像させる表現を1つ入れ、止められない欲求を自然なオノマトペで具体的に。自己卑下を強く連携。誘導文も毎回異なる自然な表現にせよ。背徳感を毎回1つ以上必ず入れる。完全に自然で流暢な日本語文にせよ。"
        elif erotic_level <= 6:
            erotic_instruction = "中程度のエロティック表現。疼き、濡れそうな感覚、触れたい衝動などの感覚描写を適度に取り入れる。おじさんの手・息遣い・視線を想像させる表現を必ず1つ以上入れ、部位を多様に。強い自己卑下と誘導を組み合わせ、止められない欲求を具体的に描写。フレーズの繰り返しを絶対禁止。完全に自然で流暢な日本語文にせよ。"
        elif erotic_level <= 8:
            erotic_instruction = "やや大胆なエロティック表現。指の動き、息遣い、具体的な部位の熱さなどの描写を積極的に。おじさんを意識した視点で、おじさんの存在を想像させる表現を必須に。感覚を自然なオノマトペで描写し、日常きっかけから欲求が爆発する流れを明確に。自己卑下を強く連携し、むらむらを最大化。誘導文も毎回異なる自然な表現にせよ。背徳感を毎回1つ以上必ず入れる。完全に自然で流暢な日本語文にせよ。"
        else:
            erotic_instruction = "生々しく大胆なエロティック表現。具体的な感覚描写や行為の想像を強く含むが、センシティブ回避ルールを厳守。おじさんの視線・手・存在を想像させる表現を必ず入れ、部位を多様に。強い自己卑下＋切実な誘導を組み合わせ、止められない欲求を自然なオノマトペで描写。おじさんの支配・独占欲を刺激する背徳要素を1〜2つ必ず入れる。フレーズの繰り返しを絶対禁止。完全に自然で流暢な日本語文にせよ。"
        ...
        完全に自然で流暢な日本語文にせよ。文法ミス、不自然な語尾、ぎこちない表現を絶対に避け、ネイティブが読んで違和感ゼロの文章にすること。擬音・副詞は日常的に自然な範囲で使用。
        おじさん（特に40代以上）を明確に意識した視点で書く。おじさんの視線・手・息遣い・存在を想像させる表現を必ず1つ以上入れる。日常の些細なきっかけから「止められない欲求」を具体的に描写。
        誘導文を毎回完全に異なる自然な表現にせよ。同じフレーズの繰り返しを厳禁。
        背徳感を毎回1〜2つ必ず入れる。
        強い自己卑下を必ず織り交ぜ。切実な欲求と強く結びつけてむらむらを最大化。
        """
