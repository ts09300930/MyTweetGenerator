...
st.title("裏垢女子ツイート生成ツール　ver1.08（言い回し多様化最強化）")
...
if st.button("生成開始"):
    if not features or not API_KEY:
        ...
        # エロ度指示（言い回し多様化強化）
        if erotic_level <= 2:
            erotic_instruction = "エロティックさは極めて控えめ。性的表現は一切避け、孤独感や雰囲気の描写のみ。同じ表現の繰り返しを絶対禁止。各ツイートで異なる感覚表現を使え（例: 熱がじわっと広がる、むずむずする、ぞわぞわする、とろける、ひくひくする、びくんとする、じんわり熱くなる、むわっと熱い、ぞくぞくする、くすぐったいような熱さ、など）。完全に自然で流暢な日本語文にせよ。"
        elif erotic_level <= 4:
            erotic_instruction = "エロティックさは軽い暗示レベル。おじさんの視線や存在を想像させる表現を1つ入れ、止められない欲求を自然な感覚表現で具体的に描写（疼く以外の言葉を優先）。同じ表現の繰り返しを絶対禁止。自己卑下を強く連携。誘導文も毎回異なる自然な表現にせよ。背徳感を毎回1つ以上必ず入れる。完全に自然で流暢な日本語文にせよ。"
        elif erotic_level <= 6:
            erotic_instruction = "中程度のエロティック表現。おじさんの手・息遣い・視線を想像させる表現を必ず1つ以上入れ、部位を多様に。同じ表現の繰り返しを絶対禁止。自然な感覚表現を多用せよ（疼くは控えめに、他の言葉を積極的に）。強い自己卑下と誘導を組み合わせ、止められない欲求を具体的に描写。完全に自然で流暢な日本語文にせよ。"
        elif erotic_level <= 8:
            erotic_instruction = "やや大胆なエロティック表現。おじさんを意識した視点で、おじさんの存在を想像させる表現を必須に。感覚を自然なオノマトペで描写し、毎日異なる言葉を選べ（同じ表現の繰り返しを絶対禁止）。自己卑下を強く連携し、むらむらを最大化。誘導文も毎回異なる自然な表現にせよ。背徳感を毎回1つ以上必ず入れる。完全に自然で流暢な日本語文にせよ。"
        else:
            erotic_instruction = "生々しく大胆なエロティック表現。具体的な感覚描写や行為の想像を強く含むが、センシティブ回避ルールを厳守。おじさんの視線・手・存在を想像させる表現を必ず入れ、部位を多様に。同じ表現の繰り返しを絶対禁止。自然な感覚表現を多用せよ（疼くは控えめに、他の言葉を積極的に）。強い自己卑下＋切実な誘導を組み合わせ、止められない欲求を描写。おじさんの支配・独占欲を刺激する背徳要素を1〜2つ必ず入れる。完全に自然で流暢な日本語文にせよ。"

        # 多様性・奥行き指示（言い回し多様化を最強化）
        variety_instruction = """
        すべてのツイートで内容、表現、シチュエーション、言い回し、感情描写を完全に多様化せよ。
        複数日分生成時も、ユーザーが指定した特徴・テーマを厳守しつつ、表現・言い回し・細かなきっかけ・誘導文だけを毎日完全にユニークにせよ。過去ツイートとの重複を絶対禁止。各ツイートは独立した新鮮な内容に。
        同じフレーズ・同じオノマトペ・同じ感覚表現・同じ誘導文の繰り返しを絶対禁止。毎日異なる言葉・言い回しを選べ。
        感覚表現は多様にせよ（例: 熱がじわっと広がる、むずむずする、ぞわぞわする、とろける、ひくひくする、びくんとする、じんわり熱くなる、むわっと熱い、ぞくぞくする、くすぐったいような熱さ、胸がざわつく、肌が火照る、息が上がる、指先が震える、体が勝手に反応する、など）。「疼く」「疼き」は控えめにし、他の表現を積極的に混ぜよ。
        ...
        誘導文を毎回完全に異なる自然な表現にせよ。同じフレーズの繰り返しを厳禁（例: 「おじさんしかいないんです…」「今すぐ助けてください…」「こんな私を、おじさんが溶かしてくれないと…」「我慢できないから声かけて…」「この熱、おじさんだけに…」「今だけは許して…」「こんな私を抱きしめて…」「温めてほしいんです…」「リプで慰めて…」「優しく触れてくれませんか…」「今夜だけは…」「おじさんの胸に飛び込みたい…」「どうか許して…」「私を満たして…」など）。
        背徳感を毎回1〜2つ必ず入れる（例: 「人目があるのに我慢できない」「夜中で一人なのに熱くなって」「誰かに見られそうで興奮」「こんな臭い体なのに」「秘密の場所で熱くなる」など）。
