...
if st.button("生成開始"):
    if not features or not API_KEY:
        ...
        # エロ度指示（分析に基づく強化: 具体的な描写を追加）
        if erotic_level <= 2:
            erotic_instruction = "エロティックさは極めて控えめ。性的表現は一切避け、孤独感や雰囲気の描写のみ。"
        elif erotic_level <= 4:
            erotic_instruction = "エロティックさは軽い暗示レベル。体が熱くなる、ドキドキするなどの間接表現のみ。おじさんの視線や存在を想像させる表現を1つ入れ、止められない欲求をじわっと・びくんなどのオノマトペで具体的に。"
        elif erotic_level <= 6:
            erotic_instruction = "中程度のエロティック表現。疼き、濡れそうな感覚、触れたい衝動などの感覚描写を適度に取り入れる。おじさんの手・息遣い・視線を想像させる表現を必ず1つ以上入れ、部位を多様に（胸・太もも・腰など偏らせず）。強い自己卑下と誘導を組み合わせ、止められない欲求を具体的に描写。"
        elif erotic_level <= 8:
            erotic_instruction = "やや大胆なエロティック表現。指の動き、息遣い、具体的な部位の熱さなどの描写を積極的に。おじさんを意識した視点で、おじさんの存在を想像させる表現を必須に。感覚をぞわぞわ・とろーりなどの生々しいオノマトペで描写し、日常きっかけから欲求が爆発する流れを明確に。"
        else:
        ...
        if self_deprecation_level <= 3:
            ...
            deprecation_instruction = "強い自己卑下・コンプレックス強調で共感を誘う（例: 「私みたいなの相手してくれる？」「こんな私でもいいですか？」）。切実な欲求と組み合わせる。"
